  ...
  try {
    ...
    if (company_id) {
      result = await db.query(
        'SELECT * FROM insurance WHERE company_id = $1 ORDER BY created_at DESC',
        [company_id]
      );
    } else {
      result = await db.query('SELECT * FROM insurance ORDER BY created_at DESC');
    }
...
router.post('/', requireAuth, enforceCompanyAccess, async (req, res) => {
  ...
  try {
    const result = await db.query(
      'INSERT INTO insurance (company_id, data) VALUES ($1,$2) RETURNING *',
...
router.put('/:id', requireAuth, async (req, res) => {
  const { data } = req.body;
  try {
    const existing = await db.query('SELECT company_id FROM insurance WHERE id = $1', [req.params.id]);
    ...
    const result = await db.query(
      'UPDATE insurance SET data=$1 WHERE id=$2 RETURNING *',
...
// (Als Teil des insurance-Moduls, aber eigene Route in server.js)
